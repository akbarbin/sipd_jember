...
class Master_tabular_performance extends Admin_Controller {
  ...
  public function view() {
    ...
    $this->data['title'] = 'Data Kinerja ' . $master_tabular[0]->name;
    $this->load->view('layout/admin', $this->data);
  }
  ...
  public function add() {
    ...
    if ($this->form_validation->run('name_validation')) {
      $data = array_merge($this->input->post(), array('type' => 'kinerja'));
      $save = $this->Master_tabular_model->save($this->set_data_before_update($data));
      $this->error_message('insert', $save);
      redirect('admin/master_tabular_performance');
    } else {
      $this->data['title'] = 'Tambah Kinerja';
  ...
  public function edit() {
    ...
    if ($this->form_validation->run('name_validation')) {
      ...
      if (!empty(self::$id)) {
        ...
        $this->data['title'] = 'Edit Kinerja ' . $master_tabular[0]->name;
        $this->load->model('Unit_model');
        $this->data['unit_list'] = $this->get_list($this->Unit_model->get_all());
        $this->load->view('layout/admin', $this->data);
      } else {
  ...
  public function generate() {
    ...
    if (!empty($post)) {
      ...
      $this->data['title'] = 'Generate Kinerja Kecamatan';
      $this->load->view('layout/admin',  $this->data);
    }
  }
