...
class Tabular_performance extends Sub_District_Controller {
    ...
    if ($this->form_validation->run('search_tabular')) {
      $this->data['tabulars'] = $this->Tabular_model->get_ancestry_depth(
              array(
                  ...
                  'tabulars.type' => 'kinerja'));

      $this->data['title'] = 'Data Kinerja Tahun ' . $post['year'];
    } else {
      $this->data['title'] = 'Data Kinerja Kecamatan';
    }

    $tabular_years = $this->Tabular_model->get_years(array('type' => 'kinerja', 'sub_district_id' => $this->get_login_active_sub_district_id()));
  ...
  public function view() {
    ...
    $this->data['title'] = 'Data Kinerja ' . $tabular[0]->name . ' Tahun ' . $tabular[0]->year;
    $this->load->view('layout/sub_district', $this->data);
  }
  ...
  public function edit() {
    ...
    if (empty($post)) {
      ...
      redirect('sub_district/tabular_performance/view/' . self::$id);
    }
  }
