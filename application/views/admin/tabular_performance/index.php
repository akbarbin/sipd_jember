...
if (!empty($tabular_years)) {
  ...
  if (isset($tabulars)) {
    ...
    echo bootstrap_table_head(array('Kriteria', 'Nilai', 'Satuan', 'Sumber Data', 'Aksi'));
    ...
    foreach ($tabulars as $key => $tabular) {
      ...
      $actions = array(
          'view' => array('name' => 'Detail', 'action' => 'view/' . $tipe . '/' . $tabular->id),
          'edit' => array('name' => 'Edit', 'action' => 'edit/' . $tipe . '/' . $tabular->id),
      );
