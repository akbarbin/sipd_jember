...
echo form_open('sub_district/tabular_general/edit/' . $id);
echo bootstrap_tag_open('table', array('class' => 'table table-bordered table-striped table-hover bg-white'));
echo bootstrap_table_head(array('Data Umum', 'Nilai', 'Satuan', 'Sumber Data'));
...
foreach ($tabulars as $key => $tabular) {
  ...
  echo bootstrap_tag_open('td');
  if (!$tabular->is_parent) {
    echo form_hidden('data[' . $tabular->id . '][value][before]', $tabular->value) . form_input('data[' . $tabular->id . '][value][after]', $tabular->value, 'class="span12"');
  }
  echo bootstrap_tag_close('td');
  echo bootstrap_tag('td', $tabular->unit_name);
  echo bootstrap_tag_open('td');
  if (!$tabular->is_parent) {
    echo form_hidden('data[' . $tabular->id . '][data_source_id][before]', $tabular->data_source_id) . form_dropdown('data[' . $tabular->id . '][data_source_id][after]', $data_sources, $tabular->data_source_id, 'class="span12"');
  }
